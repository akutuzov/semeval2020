...
if __name__ == '__main__':
    ...
    for word in sorted(target_words):
        frequency = np.sum([array0[word].shape[0], array1[word].shape[0]])

        if array0[word].shape[0] < args.min_freq or array1[word].shape[0] < args.min_freq:
            logger.info('{} omitted because of low frequency'.format(word))
            if args.f:
                print('\t'.join([word, '1', str(frequency)]), file=f_out)
            else:
                print('\t'.join([word, '1']), file=f_out)
            continue
